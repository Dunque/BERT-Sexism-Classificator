...
modelPath = absolutePath + "models/multiEngBertSmote/"
...
def train_model_hyperparams(config, checkpoint_dir=None):
    ...
    x_res_train, y_res_train = sm.fit_resample(train_inputs, y_train)

    x_res_val, y_res_val = sm.fit_resample(val_inputs, y_val)

    # DATALOADER
    # Convert lists to tensors
    x_res_train = torch.tensor(x_res_train)
    train_masks = torch.tensor(train_masks)

    # Convert lists to tensors
    x_res_val = torch.tensor(y_res_val)
    val_masks = torch.tensor(val_masks)

    # Convert other data types to torch.Tensor
    train_labels = torch.tensor(y_res_train)
    val_labels = torch.tensor(y_res_val)

    # Create the DataLoader for our training set
    train_data = TensorDataset(x_res_train, train_masks, train_labels)
    train_sampler = RandomSampler(train_data)
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=config["batch_size"])

    # Create the DataLoader for our validation set
    val_data = TensorDataset(x_res_val, val_masks, val_labels)
...
def data_loaders(config):
    ...
    x_train, x_val, y_train, y_val = train_test_split(x_res, y_res, test_size=0.1, random_state=2020)
