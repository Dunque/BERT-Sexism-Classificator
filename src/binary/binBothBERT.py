...
modelPath = absolutePath + "models/binBothBert/"
...
def load_data(translated_data=translated_data_path,
    ...
    # English version
    eng_data = data[['id', 'English', 'LabelTask1']]
    eng_data = eng_data.rename(columns={'id': 'id', 'English': 'tweet', 'LabelTask1': 'label'})

    # Spanish version
    esp_data = data[['id', 'Spanish', 'LabelTask1']]
    esp_data = esp_data.rename(columns={'id': 'id', 'Spanish': 'tweet', 'LabelTask1': 'label'})

    # Join both dataframes
    data = pd.concat([eng_data, esp_data], axis=0)
    ...
    # English version
    test_eng_data = test_data[['id', 'English']]
    test_eng_data = test_eng_data.rename(columns={'id': 'id', 'English': 'tweet'})

    # Spanish version
    test_esp_data = test_data[['id', 'Spanish']]
    test_esp_data = test_esp_data.rename(columns={'id': 'id', 'Spanish': 'tweet'})

    # Join both dataframes
    test_data = pd.concat([test_eng_data, test_esp_data], axis=0)
...
def train_model(config, train_dataloader, val_dataloader):
    ...
    for epoch_i in range(config["epochs"]):
        ...
        evaluate(model, device, val_dataloader, avg_train_loss, time_elapsed, epoch_i)
...
# EVALUATION
def evaluate(model, device, val_dataloader, avg_train_loss, time_elapsed, epoch_i):
    """After the completion of each training epoch, measure the model's performance
    on our validation set.
    """
...
def main(save_model, train_whole):
    ...
    train_dataloader, val_dataloader, y_true = data_loaders(best_trial.config)

    bert_classifier = train_model(best_trial.config, train_dataloader, val_dataloader)
    ...
    # if save_model:
    #     # Save the model
    #     bert_classifier.save("models/binEngBERT")
